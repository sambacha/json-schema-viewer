(function($) {
  ...
  function loadSchemaBySlug(slug) {
    ...
    $.getJSON(datasetUrl, function(dataset) {
      $('#jq-console').keyup($.debounce(250, function() {
        var jqExpression = $(this).val();
        try {
          $("#jq-console-output").text(JSON.stringify(jq(dataset, jqExpression), null, 2));
        } catch(e) {
          console.log(e);
          $("#jq-console-output").text("ERROR: " + e);
        }
      }));
    });
  }
